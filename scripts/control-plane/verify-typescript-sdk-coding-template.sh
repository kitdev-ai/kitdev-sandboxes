...
template_absent() {
  local code
  [[ "$template_name" =~ ^kitdev-coding-template-[0-9a-f]{12}$ ]] || return 0
  code="$(curl --disable --config "$stage/api.curlrc" --silent --show-error \
...
cleanup() {
  ...
  if [[ -n "$stage" && -f "$stage/config/e2b-template-name" ]]; then
    template_name="$(read_state_id "$stage/config/e2b-template-name" 'kitdev-coding-template-[0-9a-f]{12}\n' 36 2>/dev/null)" || template_name='invalid'
  fi
  if [[ "$template_id" =~ ^[a-z0-9]{16,32}$ && -f "$stage/api.curlrc" ]]; then
    ...
    [[ "$code" == 204 || "$code" == 404 ]] || status=1
  elif [[ "$template_name" =~ ^kitdev-coding-template-[0-9a-f]{12}$ && -f "$stage/api.curlrc" ]]; then
    for attempt in {1..5}; do
      code="$(curl --disable --config "$stage/api.curlrc" --silent --show-error \
        --output /dev/null --write-out '%{http_code}' --max-time 30 \
  ...
  if [[ "$template_name" =~ ^kitdev-coding-template-[0-9a-f]{12}$ && -f "$stage/api.curlrc" ]]; then
...
main() {
  ...
  stage="$(mktemp -d /run/kitdev-sandboxes/typescript-sdk-coding-template.XXXXXXXX)"
  chmod 0700 -- "$stage"
  ...
  uuid="${uuid//-/}"
  template_name="kitdev-coding-template-${uuid:0:12}"
  ...
  docker run --rm --pull never --platform linux/amd64 --network host --user 0:0 \
    ...
    "$NODE_IMAGE" node coding-template.ts
  printf 'status=pass operation=verify-typescript-sdk-coding-template\n'
}
