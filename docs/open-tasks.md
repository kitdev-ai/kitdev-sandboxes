...
| 5 | Runtime admission control | In progress; patch live-unproved | Enforce bounded CPU, RAM, HugeTLB, NBD, disk, concurrent build, and concurrent sandbox budgets before mutation; prove refusal, release, crash cleanup, and no overcommit under load | Depends on 3 and measurements from 4. The hugepage pool already refuses cleanly per request ([capacity](research/host-capacity-qualification-2026-08-08.md)), but that is the hardware refusing, not admission control. Blocks 9 and production claims. |
| 6 | DNS-01 wildcard HTTPS ingress | Live; renewal cycle and failure rollback unobserved | Issue and renew a trusted DNS-01 wildcard certificate; publish HTTPS in the selected firewall mode; prove API and wildcard names, renewal/reload, failure rollback, 80 closed, and no public internal ports | Certificate issued and ingress live in public mode; 80 and all internal ports refused externally. A real renewal and a failure rollback are still unobserved. [Evidence](research/external-https-enablement-2026-08-08.md). |

## Next

| # | Workstream | Status | Objective completion gate | Dependencies / evidence |
|---:|---|---|---|---|
| 7 | External official SDK matrix | Passed from a development client; product server outstanding | From a client host, pinned `e2b@2.38.0` passes auth, create/list/connect/info/metrics/timeout/kill, commands, PTY, files/watch, pause/resume, snapshots, guest HTTP, WebSocket/streaming, concurrency refusal, and cleanup | 42 of 42 checks passed over public HTTPS on 2026-08-08. Runner: `scripts/external-sdk-matrix`. The same run from the product bare-metal server remains outstanding. |
...
| 14 | Host capacity ceiling | Measured; sustained load unproved | Concurrency is `hugepage pool / per-sandbox RAM`: 12 x 2 GiB or 3 x 8 GiB measured, with clean per-request refusal past the pool. Remaining: sustained multi-hour load, vCPU contention at full fleet, and a decision on raising the pool to the 32 GiB policy ceiling | Depends on 3 for any pool change. [Evidence](research/host-capacity-qualification-2026-08-08.md). |
