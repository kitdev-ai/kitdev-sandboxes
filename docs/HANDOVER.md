...
| 6 | Fresh-host seed template | **Blocks fresh install** | `seed-local-template.sh` needs far more than a directory: it pins exact size and sha256 for eleven blobs totalling about 1.63 GB, including five ancestor rootfs images, and `publish-template-dirs.py` is a closed allowlist of exactly those six historical UUIDs. Nothing builds, downloads or hashes any of them, so the publish helper cannot serve a freshly built template either. `verify-api-proxy-e2e.sh` pins the same lab build ID, so `kitdev test-core` stays broken independently. Fixing this will also expose whether `local-dev-team` has an in-repo creator |
